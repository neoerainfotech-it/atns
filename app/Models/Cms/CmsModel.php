...
class CmsModel extends Model
{
    ...
    public function save_home_heading($data)
    {
        ...
        // 1. SAFELY BACKUP EXISTING IMAGES IN MEMORY BEFORE EXECUTING DELETIONS
        // --- SECTION 2: HERO BANNER SLIDESHOW CAROUSEL (🌟 FIXED REALIGNMENT) ---
        // 1. SAFELY BACKUP EXISTING IMAGES IN MEMORY BEFORE EXECUTING DELETIONS
        $cachedImages = [];
        $oldSliders = $this->db->table('home_slider')->where('home_id', $home_id)->get()->getResult();
        foreach ($oldSliders as $oldSlider) {
            $cachedImages[$oldSlider->id] = $oldSlider->image;
        }

        // 2. NOW SAFE TO CLEAR THE TABLE FOR RE-ALIGNMENT ROW REWRITES
        ...
        if (!empty($data['slide_title'])) {
            ...
            for ($i = 0; $i < $num; $i++) { 
                ...
                $currentSlideId = $data['slide_id'][$i] ?? '';
                
                // 3. PROPERLY ASSIGN IMAGES USING THE ARRAYS GENERATED BY THE CONTROLLER
                if (empty($currentSlideId)) {
                    // It's a brand new slide: Grab the new path generated by the controller
                    $array['image'] = $data['slide_image_new'][$i] ?? '';
                } else {
                    // It's an existing slide: Check if a new file was uploaded to overwrite it
                    if (!empty($data['slide_image_update'][$currentSlideId])) {
                        $array['image'] = $data['slide_image_update'][$currentSlideId];
                    } else {
                        // No new file uploaded: Restore the original image from the memory cache
                        $array['image'] = $cachedImages[$currentSlideId] ?? '';
                    }
                }
