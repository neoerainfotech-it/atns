...
class CmsModel extends Model
{
    ...
    /**
     * Handles Homepage CMS Saving Routines
     */
    public function save_home_heading($data)
    {
        $array = [];
        ...
        // --- SECTION 1: VISION MATRIX COUNTERS ---
        ...
        // --- SECTION 2: HERO BANNER SLIDESHOW CAROUSEL (🌟 FIXED REALIGNMENT) ---
        // Explicitly matched table reference against your cyb_home_slider schema layout 
        $query2 = $this->db->table('home_slider');
        $query2->where('home_id', $home_id)->delete();

        if (!empty($data['slide_title'])) {
            $num = count($data['slide_title']);
            
            for ($i = 0; $i < $num; $i++) { 
                $array = [];
                $array['home_id'] = $home_id;
                
                // Track newly generated asynchronous file uploads arrays cleanly
                if (!empty($data['slide_image_new'][$i])) {
                    $array['image'] = $data['slide_image_new'][$i];
                } elseif (!empty($data['slide_id'][$i]) && isset($_FILES['slide_image_' . $data['slide_id'][$i]])) {
                    $file = request()->getFile('slide_image_' . $data['slide_id'][$i]);
                    if ($file && $file->isValid() && !$file->hasMoved()) {
                        $newName = $file->getRandomName();
                        if ($file->move('uploads/images/', $newName)) {
                            $array['image'] = 'uploads/images/' . $newName;
                        }
                    }
                }
                
                // Keep the existing target image path if no replacement asset is uploaded
                if (empty($array['image']) && !empty($data['slide_id'][$i])) {
                    $oldRow = $this->db->table('home_slider')
                                       ->where(['id' => $data['slide_id'][$i]])
                                       ->get()->getRow();
                    $array['image'] = $oldRow ? $oldRow->image : '';
                }

                $array['title']       = $data['slide_title'][$i] ?? '';
                $array['description'] = $data['slide_description'][$i] ?? '';
                $array['link']        = $data['slide_link'][$i] ?? '';
                $array['sort_order']  = (int)($data['slide_sort_order'][$i] ?? 0);
    ...
    /**
     * Handles About Page CMS Saving Routines
     */
    public function save_about_heading($data)
    {
        $array = [];
    ...
    /**
     * Handles Services Page CMS Saving Routines
     */
    public function save_service_heading($data)
    {
        $array = [];
    ...
    /**
     * Handles Technology Components CMS Saving Routines
     */
    public function save_technology($data)
    {
        $array = [];
    ...
    public function save_electronic($data)
    {
        $array = [];
    ...
    public function save_infrastructure($data)
    {
        $array = [];
    ...
    public function save_sustainability($data)
    {
        $array = [];
    ...
    public function save_cx_heading($data)
    {
        $array = [];
    ...
    public function get_global_presence_list()
    {
    ...
    public function save_cx_feature($data)
    {
