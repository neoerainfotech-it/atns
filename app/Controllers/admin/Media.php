...
{
    // Declared class properties to support strict property mutations in modern PHP
    public $config_logo;
    ...
    public function __construct()
    {
        ...
        $this->config_logo = isset($default_img->value) ? $default_img->value : ''; 
    }
    ...
    public function add_blog($id = false)
    {
        ...
        if(!empty($id)) {
            ...
            // Fixed the extra parameter error constraint here 
            ...
            // FIXED BOUND VARIABLE: Maps your custom database column to the form template layer variable name
            $data['author_name'] = isset($row->author_name) ? $row->author_name : '';
            ...
            // Added safe empty fallback for baseline create requests
            $data['author_name'] = '';
            $data['readingTime'] = '';
        ...
        if($this->request->getMethod() === 'POST'){
            if($this->validate($rules)==false){
                ...
                // FIXED VARIABLE LOOKUP: Explicitly captures from frontend input named 'author_name'
                $save['author_name'] = $this->request->getVar('author_name');
                ...
                if(!empty($id)){
                    ...
                    // FIXED UPDATE LOOKUP STATEMENT: Targeted using strict scalar primary identity variable
    ...
    public function delete_blogs()
    {
        ...
        if($ids){
            ...
            session()->setFlashdata('success','Record Delete success');
        }
        return redirect()->to('admin/blogs');
    }
            ...
            if ($this->validate($rules)==false) {
                ...
                if ($id) {
                    ...
                    if ($result) {
                        ...
                        session()->setFlashdata('error','Record not update');
                        return redirect()->to('admin/add_blog_category/'.$id);
                    }
                }else{
                    $save['create_date'] = date('Y-m-d H:i:s');
                    ...
                    if ($result) {
                        session()->setFlashdata('success','Record insert successfully');
                        return redirect()->to('admin/blog_category');
                    }else{
                        session()->setFlashdata('error','Record not insert');
                        return redirect()->to('admin/blog_category');
                    }
                }
            } 
